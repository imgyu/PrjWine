   ...
   <div class="container"
      ...
      <c:forEach var="store" items="${storelist}">
         ...
         <%--     <div style="max-width: 300px; max-height: 300px; margin: 0 auto;">
                  <img src="${store.w_image}" alt="와인이미지" style="width: 100%; height: 100%; object-fit: contain;">
              </div> --%>
            <div class="card-body">
               <h5 class="card-title">${s_name}</h5>
               <p class="card-text">${s_cont}</p>
               <p class="card-text">${s_phone}</p>
               <p class="card-text">${s_address}</p>
               <p class="card-text">${s_detailaddress}</p>
               <a href="/WineInfo?s_no=${s_no }}" class="btn btn-primary">매장정보</a>
            </div>
         </div>
      </c:forEach>
   </div>
