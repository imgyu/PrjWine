	...
	<c:forEach var="board" items="${boardCont}">
		<div class="container1" id="board"
			...
			<table id="table">
				...
				<div>
					<div>
						<label>글작성자</label> <input type="text" value="${board.s_name}"
							readonly>
					</div>
			...
			<c:choose>
				<c:when
					test="${not empty sloginVo.s_no and sloginVo.s_no eq board.s_no}">
					<form action="/BoardDelete?s_no=${board.s_no}&b_idx=${board.b_idx}"
						method="POST" id="search-form">
						<button type="submit" class="btn1" id="delete-button">삭제</button>
						<a
							href="/BoardContUpdateForm?s_no=${board.s_no }&b_idx=${board.b_idx}&b_title=${board.b_title}"
							class="btn1" id="update-link">수정</a> <a type="submit"
							class="boxed-btn" onclick="goBack()">뒤로가기</a>
						<script>
							document
